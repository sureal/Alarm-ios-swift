    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if indexPath.section == 2 {
            ...
            for visibleCell in visibleCells {
                let section = tableView.indexPath(for: visibleCell)?.section
                if section == indexPath.section && visibleCell != cell {
                    visibleCell.accessoryType = UITableViewCell.AccessoryType.none
                }
            }
        }
    }
    ...
    func mediaPicker(_ mediaPicker: MPMediaPickerController,
        ...
        if !mediaItemCollection.items.isEmpty {
            ...
            if let mediaItem = self.mediaItem {
                ...
                if let mediaIdentifier = (mediaItem.value(forProperty: MPMediaItemPropertyPersistentID)) as? String {
                    self.mediaID = mediaIdentifier
